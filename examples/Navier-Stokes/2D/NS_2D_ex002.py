...
# Flow through a stepped channel
...
if __name__ == '__main__':
    ...
    simplices_coord = np.round(np.array(faces, dtype=np.float32)[:, :2]) / 100.0
    mesh = Mesh_from_FreeCAD_ns(_faces=simplices_coord)
    # Mesh data
    ...
    for point_index in mesh.boundary_points:
        ...
        if p[0] <= 0 and p[1] == 0:  # South before step
            mesh.bc_points_u["dirichlet"][point_index] = 0
            mesh.bc_points_v["dirichlet"][point_index] = 0
        if p[0] >= 0 and p[0] < x_max and p[1] < 0:  # South after step
            mesh.bc_points_u["dirichlet"][point_index] = 0
            mesh.bc_points_v["dirichlet"][point_index] = 0
        if p[1] == y_max:  # North
            mesh.bc_points_u["dirichlet"][point_index] = 0
    ...
    for ch_idx, edge_points in enumerate(mesh.tri.convex_hull):
        ...
        if p1[0] == x_max and p2[0] == x_max:  # East
            mesh.bc_points_u["neumann_edge"][ch_idx] = [0, 0]
            mesh.bc_points_v["neumann_edge"][ch_idx] = [0, 0]
        if (p1[0] <= 0 and p1[1] == 0) and (p2[0] <= 0 and p2[1] == 0):  # South before step
            mesh.bc_points_p["neumann_edge"][ch_idx] = [0, 0]
        if (p1[0] >= 0 and p1[0] < x_max and p1[1] < 0) and (p2[0] >= 0 and p2[0] < x_max and p2[1] < 0):  # South after step
            mesh.bc_points_p["neumann_edge"][ch_idx] = [0, 0]
        if (p1[1] == y_max and p1[0] > x_min and p1[0] < x_max) and (p2[1] == y_max and p2[0] > x_min and p2[0] < x_max):  # North
            mesh.bc_points_p["neumann_edge"][ch_idx] = [0, 0]
    ...
    plt.savefig('output/NS_2D_ex002.png')
    plt.close()