...
# FEM Navier-Stokes 2D solver class
class FENS2D:
    """
    class for 2D NS equations solver with finite element method
    ...
    def set_E(self):
        ...
        for i, el_ps in enumerate(self.mesh.tri.simplices):
            ...
            ubar[:, 0] = np.array([ 2*self.u[el_ps[0], 0] + self.u[el_ps[1], 0]   + self.u[el_ps[2], 0], 
                                    self.u[el_ps[0], 0]   + 2*self.u[el_ps[1], 0] + self.u[el_ps[2], 0],
                                    self.u[el_ps[0], 0]   + self.u[el_ps[1], 0]   + 2*self.u[el_ps[2], 0] ])
            
            vbar[:, 0] = np.array([ 2*self.v[el_ps[0], 0] + self.v[el_ps[1], 0]   + self.v[el_ps[2], 0], 
                                    self.v[el_ps[0], 0]   + 2*self.v[el_ps[1], 0] + self.v[el_ps[2], 0],
                                    self.v[el_ps[0], 0]   + self.v[el_ps[1], 0]   + 2*self.v[el_ps[2], 0] ])
